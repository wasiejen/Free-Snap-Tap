...
block_transfer v2 (S1 finding): probes 115 + 263 pin the LEGACY error strings, so the Part A richer non-unique format cannot be wired yet. EVIDENCE: probe 263 pins byte-exact `Error: Start marker 'DUP' is not unique in bt/bt-rep-nq.txt.` and probe 115 pins byte-exact `Error: End marker 'ZZ-END' not found after start marker.` — probes are do-not-touch in S1 (probe section = S3) and the gate must stay 297/297, so S1 kept the legacy byte-exact forms (S1 spec asked for "non-unique: match count + the first match line numbers" — conflicting with probe 263; resolved in favor of the pinned probe, documented in the worker handover). GOAL: the S3 probe unit updates the S15 probe checks (115, 263) to the v2 teaching format and the tool switches to it (non-unique with match count + first match line numbers; extraction end-not-found carrying the file reference). ACCEPTANCE: non-unique error reports count + first match lines; probes green on the new strings; smoke pins re-pinned to the same format. SCOPE: .opencode/plugin/probes/handover_probe.mjs (S15 115/263), .opencode/tools/block_transfer.ts (nonUniqueError/notFoundError + the legacy end-before-start branch), .opencode/plugin/tests/block_transfer.smoke.mjs.
