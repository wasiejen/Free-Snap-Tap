...
- **NEXT (rest of this iteration, in order):** 1. T3 spec (the `loop_log.ts` tool — Part 3 of the approved batch) written FRESH against the verified state (read Part 3 + `agent_readme_loop.md` §Loop log first) → launch fresh `worker_Q4_120K` → verify. 2. Close the batch: `2026-09-12_loop-tool-batch.md` approved → `implemented/` with the VERDICT; the Part 2 / Part 3 prompt/doc preference lines applied planner-direct AFTER green (separate bookkeeping commit); NAP + `plan10+1_summary.md` + action line. NOTE for any T3 spec: worker launches in this looprun have died at `context_length_exceeded` (T2 launch; cf. iter-3) — if the T3 launch dies the same way, RESCUE planner-direct if the artifacts are verifiable, else a WIP-rescue commit + slimmer follow-up.
