...
## `context.client` is UNDEFINED in this host build
- **Do:** do not rely on `context.client` (or `context.api`) for SDK calls in
  this build. `context.api` is NOT necessarily SDK-client-interchangeable.
  Prefer direct context fields; if a client is required, guard with a clear
  error, not a silent fallback.
- **Why (evidence):** probe (modified session_info) threw `undefined is not an
  object (evaluating 'context.client.app')` (2026-09-12). This is why
  compact_memory's client path fails here → it fell to the HTTP fallback → no
  listener → "Unable to connect" (TODO #52).
- **Ref:** probe in session ses_f6b7c5242ffeZpNl0Ar8mILWua; TODO #52.
- **Keys:** context.client, context.api, undefined, SDK client, compact, guard.
