# TASK ÔÇö loop.log protocol (prompt-only): `agent_readme_loop.md` section + 3 prompt reference lines

FIRST read `AGENTS.md`, `agents_repo.md`, and this file. Source of the
protocol: maintainer inbox `2026-09-11_02-03` (in `maintainer/done/`) ÔÇö a
...
a SEPARATE prompt-only task ÔÇö the protocol text lives in
`agent_readme_loop.md` (single source of truth), the 3 live prompts get a
SHORT REFERENCE LINE ONLY ÔÇö reference, never restate (AGENTS.md
┬ºRole-and-interaction-model rule: prompts reference shared sections by name).
This is **meta-only** work: no FST code, no plugin, no observable behavior
change ÔÇö pre-approved class.

## The protocol (what the section must specify)
- **Log location:** ONE file per looprun, in that looprun's autorun archive
  folder ÔÇö `.opencode/archive/autorun-<YYYY-MM-DD_HH-MM>/loop_log.md` (append
  only; created on first write).
- **Three line types** (fields the maintainer named ÔÇö pick ONE consistent
  line format for all three and show an example in the section):
  - `START` ÔÇö written by looprunner, planner, and worker, each at its own
    session/task start: `date_time`, `session_id`, `agent_model`,
    task-oneliner.
  - `RETURN` ÔÇö written by planner and looprunner, each when a sub-agent
    returns: same triple (date_time, the returned agent's session_id,
    agent_model).
  - `DONE` ÔÇö written by EVERY agent on task completion, additionally carrying
    the final gauge readout (`<CTX>%/<REM>K` ÔÇö verbatim from the gauge
...
1. `.opencode/system_prompts/agent_readme_loop.md` ÔÇö add a `## Loop log`
   ...
   protocol above.
2. `.opencode/system_prompts/agents/prompt_agent_planner.md` ÔÇö ONE short
   reference line pointing at the Loop-log section (e.g. inside the
   Instruction-index or the Autonomous-mode block ÔÇö where it reads naturally).
3. `.opencode/system_prompts/agents/prompt_agent_task.md` ÔÇö ONE short
   reference line (same rule).
4. `.opencode/system_prompts/agents/prompt_agent_looprunner.md` ÔÇö ONE short
   reference line (same rule).

The reference lines must NOT restate the protocol (no field lists, no line
formats) ÔÇö one sentence max each, naming the section.

## Out of scope
- Do NOT create `loop_log.md` now ÔÇö the protocol takes effect from the NEXT
...
Per AGENTS.md ┬ºContext budget ÔÇö self-gauge `node .opencode\plugin\scripts\peek.mjs`
between chunks and after the commit; stop line `REM Ôëñ 15k` or `ÔëÑ 85 %`.
Findings you cannot fix in scope ÔåÆ APPEND to `todo_inbox.md` (never
`TODO.md`).
