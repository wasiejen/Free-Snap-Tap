...
WM_KEYUP = 0x0101
EXIT_KEY = 35 # END key
TOGGLE_ON_OFF_KEY = 46 # DELETE key
PRESS = True
RELEASE = False
...
def which_key_to_send(group_index):
    ...
    # sum of values in group of group_indexa aka number of keys pressed at this time
    num_of_keys_pressed = sum(list(tap_groups_states_dict[group_index].values()))
    #if DEBUG: 
    print("tap group state: ", tap_groups_states_dict[group_index].values())
