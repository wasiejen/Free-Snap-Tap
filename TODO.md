...
- **Status:** OPEN (maintainer-ordered next unit 2026-09-24). Architecture ruling: host auto-compaction stays DEACTIVATED by design (`"auto": false` — it fired uncontrollably, `buffer` never worked); the host `compaction` config only supplies defaults (model + keep); the agent-driven compaction stays primary; the emergency compact is the remaining piece. NOTE: keep-values drift — the host config says `keep.messages: 18` but `compact_budget.json` says `keepMessages: 12` (tokens agree: 30000) — confirm which is the intended default.
