...
- **Status:** LANDED — the commit hash is recorded in the PLANNER's bookkeeping commit (the commit-hash rule; not in this entry). LIVE acceptance pending: the plugin activates on the next host restart; the planner verifies the first named spawn (out of scope for the worker).
