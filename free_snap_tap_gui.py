...
#import free_snap_tap2 as logic
...
def which_key_to_send(group_index):
    ...
    """
    if DEBUG: print(tap_groups_states_dict[group_index])
...
def win32_event_filter(msg, data):
    ...
    # Toggle pause/resume if the DELETE key is released
    elif vk_code == TOGGLE_ON_OFF_KEY and msg == WM_KEYUP:
        global PAUSE
        PAUSE = not PAUSE
        print('--- {} ---'.format('resumed' if not PAUSE else 'paused'))
...
def add_tap_group(new_group):
    ...
    tap_groups_states_dict.append({vk_codes_dict[key]: 0 for key in new_group if key in vk_codes_dict})
...
def delete_tap_group(index):
    ...
    del tap_groups_states_dict[index]
    del tap_groups_last_key_pressed[index]
    del tap_groups_last_key_send[index]



def update_tap_groups_listbox():
    """
    Update the Listbox to display current tap groups.
    """
    tap_groups_listbox.delete(0, tk.END)
    for group in tap_groups:
        tap_groups_listbox.insert(tk.END, ', '.join(group))

def add_tap_group():
    """
    Add a new tap group based on user input.
    """
    new_group = new_group_entry.get().split(',')
    if new_group:
        add_tap_group(new_group)
        update_tap_groups_listbox()
        new_group_entry.delete(0, tk.END)

def delete_tap_group():
    """
    Delete the selected tap group.
    """
    selected_index = tap_groups_listbox.curselection()
    if selected_index:
...
root = tk.Tk()
root.title("Snap-Tapping Configuration")

# Create Listbox to display tap groups
tap_groups_listbox = tk.Listbox(root, selectmode=tk.SINGLE)
tap_groups_listbox.pack(padx=10, pady=10, fill=tk.BOTH, expand=True)

# Create Entry widget to input new tap groups
new_group_entry = tk.Entry(root)
new_group_entry.pack(pady=5)

# Create "Add" and "Delete" buttons
add_button = tk.Button(root, text="Add Tap Group", command=add_tap_group)
add_button.pack(side=tk.LEFT, padx=5)

delete_button = tk.Button(root, text="Delete Tap Group", command=delete_tap_group)
